...
def load_model_results():
    ...
    user_segments = pd.read_csv('segmentation/user_segments.csv')
    transactions = pd.read_csv('output/processed_transactions.csv')
    
    # Load model
    model = pickle.load(open('train-model/purchase_prediction_model.pkl', 'rb'))
    model_features = pickle.load(open('train-model/model_features.pkl', 'rb'))
...
def plot_confusion_matrix(user_features, model, model_features):
    ...
    plt.tight_layout()
    plt.savefig('pngs/confusion_matrix.png', dpi=300, bbox_inches='tight')
...
def plot_feature_importance(model, model_features):
    ...
    plt.tight_layout()
    plt.savefig('pngs/feature_importance.png', dpi=300, bbox_inches='tight')
...
def plot_customer_segments(user_segments):
    ...
    plt.tight_layout()
    plt.savefig('pngs/customer_segments.png', dpi=300, bbox_inches='tight')
...
def plot_transaction_distribution(transactions):
    ...
    plt.tight_layout()
    plt.savefig('pngs/transaction_distribution.png', dpi=300, bbox_inches='tight')
...
def plot_correlation_heatmap(user_features):
    ...
    plt.tight_layout()
    plt.savefig('pngs/correlation_heatmap.png', dpi=300, bbox_inches='tight')
...
def plot_segment_distribution(user_segments):
    ...
    plt.tight_layout()
    plt.savefig('pngs/segment_distribution.png', dpi=300, bbox_inches='tight')
