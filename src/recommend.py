...
def load_data():
    ...
    user_item_matrix = pd.read_csv('featured-eng/user_item_matrix.csv', index_col=0)
...
def save_recommendation_model(item_similarity_df):
    ...
    item_similarity_df.to_csv('recommend/item_similarity_matrix.csv')
    pickle.dump(item_similarity_df, open('recommend/item_similarity.pkl', 'wb'))
