...
def save_preprocessed_data(unified_df, transactions, clickstream):
    ...
    unified_df.to_csv('output/processed_data.csv', index=False)
    transactions.to_csv('output/processed_transactions.csv', index=False)
    clickstream.to_csv('output/processed_clickstream.csv', index=False)
